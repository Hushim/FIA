...
void FilterManager::ApplyChannel(PixelBuffer ** display_buffer) {
  ...
  FilterRBG * filter_rgb = new  FilterRBG(channel_color_red_,
                                          channel_color_blue_,
                                          channel_color_green_);
  filter_rgb->apply_filter_to_photo(*display_buffer);
}
...
void FilterManager::ApplySaturate(PixelBuffer ** display_buffer) {
  ...
  FilterSaturation * saturation_filter =
  new FilterSaturation(saturation_amount_);
  saturation_filter->apply_filter_to_photo(*display_buffer);
}
...
void FilterManager::ApplyBlur(PixelBuffer ** display_buffer) {
  ...
  Filter * blur_filter = new FilterBlur(blur_amount_);
  blur_filter->apply_conv_filter_to_photo(*display_buffer);
}
...
void FilterManager::ApplySharpen(PixelBuffer ** display_buffer) {
  ...
  Filter * sharp_filter = new FilterSharpen(sharpen_amount_);
  sharp_filter->apply_conv_filter_to_photo(*display_buffer);
}
...
void FilterManager::ApplyMotionBlur(PixelBuffer ** display_buffer) {
  ...
  new FilterMotionBlur(motion_blur_amount_, motion_blur_direction_);
  motion_blur_filter->apply_conv_filter_to_photo(*display_buffer);
}

void FilterManager::ApplyEdgeDetect(PixelBuffer ** display_buffer) {
  std::cout << "Apply has been clicked for Edge Detect" << std::endl;

  Filter * border_detection_filter = new FilterBorderDetection(5);
  border_detection_filter->apply_conv_filter_to_photo(*display_buffer);
}
...
void FilterManager::ApplyQuantize(PixelBuffer ** display_buffer) {
  ...
  FilterQuantize * quantize_filter = new FilterQuantize(quantize_bins_);
  quantize_filter->apply_filter_to_photo(*display_buffer);
}
void FilterManager::ApplyThreshold(PixelBuffer ** display_buffer) {
  ...
  FilterThreshold * threshold_filter = new FilterThreshold(threshold_amount_);
  threshold_filter->apply_filter_to_photo(*display_buffer);
}
void FilterManager::ApplySpecial(PixelBuffer ** display_buffer) {
  std::cout << "Apply has been clicked for Special" << std::endl;
  FilterSpecial * special_filter = new FilterSpecial();
  special_filter->apply_filter_to_photo(*display_buffer);
}
