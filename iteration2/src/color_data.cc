...
ColorData ColorData::clamped_color(double factor, double bias) const {
        float clamped_red = ColorData::clamp_value(this->red() * factor
            + bias, 0.f, 1.f);
        float clamped_green = ColorData::clamp_value(this->green() * factor
            + bias, 0.f, 1.f);
        float clamped_blue = ColorData::clamp_value(this->blue()* factor
            + bias, 0.f, 1.f);
