...
FlashPhotoApp::FlashPhotoApp(int width, int height) : BaseGfxApp(width, height),
                                                      ...
                                                      cur_color_blue_(0.0) {
  undo_buffers_stack_ = new StateStack();
  redo_buffers_stack_ = new StateStack();
}
...
void FlashPhotoApp::LeftMouseDown(int x, int y) {
  ...
  state_manager_.SaveState(&display_buffer_,
                           undo_buffers_stack_,
                          redo_buffers_stack_);

  if (cur_tool_ == 6) {
    tools_[cur_tool_]->ApplyToStamp(x, height()-y,
                                   &display_buffer_);
  } else {
  ...
  switch (control_id) {
    ...
    case UICtrl::UI_LOAD_CANVAS_BUTTON:
      state_manager_.SaveState(&display_buffer_,
                           undo_buffers_stack_,
                          redo_buffers_stack_);
      std::cout << io_manager_.file_name() << std::endl;
      io_manager_.LoadImageToCanvas(io_manager_.file_name(), &display_buffer_);
      SetWindowDimensions(
                        display_buffer_ -> width(),
                        display_buffer_ -> height());
      // sleep(1000);
      break;
    case UICtrl::UI_LOAD_STAMP_BUTTON:
      TStamp::stamp_buffer_ =  io_manager_.LoadImageToStamp(
        io_manager_.file_name());
      TStamp::SetStampBuffer(TStamp::stamp_buffer_);
      break;
    case UICtrl::UI_SAVE_CANVAS_BUTTON:
      io_manager_.SaveCanvasToFile(io_manager_.file_name(), &display_buffer_);
...
// void FlashPhotoApp::SetWindowDimensions (int width, int height){
//   height_ = height;
//   width_ = width;
