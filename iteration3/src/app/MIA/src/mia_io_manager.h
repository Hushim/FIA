...
class MIAIOManager : public IOManager {
 public:
  ...
  void LoadImageToMarker(PixelBuffer **buffer,
  ...
  std::string image_name_plus_seq_offset_for_cmd_mia(const std::string& filename,
                                                                  int offset);
  void LoadImageToBuffer(const std::string &name, PixelBuffer **buffer);
