...
void MIAIOManager::LoadImageToMarker(PixelBuffer **buffer,
                                 const std::string & file_name) {
  *buffer = ImageHandler::LoadImage(file_name);
}

void MIAIOManager::LoadNextImage(PixelBuffer **buffer) {
  set_image_file(next_file_name_);
  LoadImageToCanvas(buffer);
}

void MIAIOManager::LoadPreviousImage(PixelBuffer **buffer) {
  set_image_file(prev_file_name_);
  LoadImageToCanvas(buffer);
}

void MIAIOManager::LoadImageToCanvas(PixelBuffer **buffer) {
...
void MIAIOManager::LoadImageToBuffer(const std::string &name, PixelBuffer **buffer) {
  //std::cout << "Load Stamp been clicked for file " <<
  //    file_name_ << std::endl;
  *buffer = ImageHandler::LoadImage(name);
  //PixelBuffer * load_buffer = new ;
  //IOManager::LoadImageToStamp(&load_buffer);
  //return load_buffer;
}

// void IOManager::SaveCanvasToFile(const std::string &name, const PixelBuffer& buffer) {
//   ImageHandler::SaveImage(name, &buffer);
// }
...
std::string MIAIOManager::image_name_plus_seq_offset_for_cmd_mia(
    const std::string& filename,
    int offset) {
